...
/**
 * Per-campaign breakdown for Website Performance — same five metrics as
 * the KPI strip, sorted descending by spend.
 *
 * The campaigns array comes pre-sorted from /api/website-performance,
 * but we sort defensively so a future shape change doesn't break the
 * "top spender first" expectation.
 */
export default function WebsiteCampaignTable({ campaigns }) {
          ...
          <tbody className="divide-y divide-neutral-100">
            {sorted.map((c) => (
              <tr key={c.campaignId} className="hover:bg-neutral-50">
                ...
                <td className="px-4 py-2.5 text-right tabular-nums">
                  {c.conversionRate === null || c.conversionRate === undefined
                    ? "—"
                    : formatPercent(c.conversionRate, { decimals: 2 })}
                </td>
              </tr>
            ))}
          </tbody>
